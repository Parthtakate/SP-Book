...
Service / Data Layer (lib/services/, lib/data/)
...
Provides Google Sign-In with transparent Auth.
Data triggers batch-writes in Firestore (atomic uploads to prevent state inconsistencies).
Backups are tied to unique User UID references in Cloud Firestore.
Conflict updates resolve via the .updatedAt schema flags if present.
...
Standard Flutter environment check (flutter doctor).
Fetch packages: flutter pub get.
Set up Firebase configuration on Android/iOS via flutterfire configure.
Ensure firestore.rules are deployed for strict user scope isolating writes strictly to request.auth.uid.
